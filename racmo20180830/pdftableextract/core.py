...
# Mark bounding box.
  ...
  def boxOfString(x,p) :
    ...
    return ([int(bitmap_resolution * float(x)) + pad for x in s[0:4] ]
...
# fork out to extract text for each cell.
  ...
  def getCell(xxx_todo_changeme ):

    (i,j,u,v) = xxx_todo_changeme
    (l,r,t,b) = ( vd[2*i+1] , vd[ 2*(i+u) ], hd[2*j+1], hd[2*(j+v)] )
    p = popen("pdftotext", 
...
def o_cells_xml(cells,pgs, outfile=None,infile=None, name=None, output_type=None) : 
  ...
  for cl in cells :
    ...
    list(map(lambda a: x.setAttribute(*a), list(zip("xywhp",list(map(str,cl))))))
    if cl[5] != "" :
