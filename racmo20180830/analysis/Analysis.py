...
#Getting number of pages in a pdf file    
 ...
 #Takes a dataframe of filegroups which also contains the list of files as its argument and returns a dataframe files with columns, name,file group and its filetype(right now its either ticket or other as the file with smallest name in a filegroup is ticket)
    ...
    def get_classify_result(file_name):
        ...
        classlis=[]
        #header keywords
        document_start=["AUTO", "DECRETO", "DILIGENCIA DE ORDENACION", "DILIGENCIA D\'ORDENACIO", "PROVIDENCIA DEL/DE LA MAGISTRADO-JUEZ",
                    "PROVIDENCIA LA MAGISTRADA JUEZ QUE LA DICTA", "PROVIDENCIA JUEZ/MAGISTRADO-JUEZ", "PROVIDENCIA DEL/DE LA JUEZ", "SENTENCIA"]
        ...
        for j, row in fdf.iterrows():
            if row['filetype']=='TICKET':
                text = unidecode.unidecode(row['text_response']).lower()
#             text=''.join(unidecode.unidecode(unicode(row['text_response'],'utf-8')).split()).lower()
            else:
                text = (row['text_response']).lower()
#             orgtext=''.join(unidecode.unidecode(row['text_response']).split())
...
#             rem_word = ''.join(parat.split()).lower()
            ...
            for i,kdrow in kdf.iterrows():
                if text[:5]!='Error':
                    ...
                    if f and (kdrow['filetype']==row['filetype'] or(kdrow['filetype']=='NOTIFICATION' or row['filetype']=='OTHER') )and (kdrow['purpose']=='CLASSIFICATION'):
            ...
            if not bool(fdf.loc[j,'keywords']):
                special_keywords = ['averiguar','facilite','despachar ejecucion','admitir','oigase']
                for i,kdrow in kdf.iterrows():
                    if text[:5]!='Error':
                        f=False
                        if(kdrow['filetype']==row['filetype'] or(kdrow['filetype']=='NOTIFICATION' and row['filetype']=='OTHER') )and (kdrow['purpose']=='CLASSIFICATION'):
                            ...
                            for k in kdrow['keyword']: 
                                kw = unidecode.unidecode(k)
                                if len(kw.lower()) < 5 or any(word in text for word in special_keywords):
                                    if not (re.search(r'\b' + kw + r'\b', text)):
                                        f=False
                                else:
                                    match=[]
