...
class Document_Analysis:    
    def keywordimport():
        ...
        keyword_df= pd.read_sql_query('SELECT k.id,fc.file_class_name asfileclass,ft.type,p.purpose_type,\
                                      k.decision_type,k.keyword_list,bia.file_class_name as bias FROM keywords k\
                                      left Join file_class fc on fc.id = k.file_class_id left Join file_class bia\
                                      on bia.id = k.bias left Join file_class f on f.id = k.file_class_id left Join\
                                      file_types ft on ft.id = k.file_type_id left Join purpose p on p.id = k.purpose_id',engine)
...
#         return text
                ...
                if row['text_response'][:5] != 'Error':
                    ...
                    if f and (kdrow['filetype']==row['filetype'])and (kdrow['purpose']=='CLASSIFICATION'):
                        ...
                        if kdrow['fileclass'] in fdf.loc[j,'keywords'].keys():
                            ...
                            print(fdf.loc[j,'keywords'],{kdrow['fileclass']:list()})
    ...
    def get_textExtract(fgdf,fdf,kdf):
        procedures={'CAM':'Cambiario','COG':'Cognicion','CON':'Concurso Acreedores','EJE':'Juicio Ejecutivo',
                    ...
                    'PTC':'Pieza Tasacion Costas','PSO':'Pieza Seperada Oposicion','AUX':'Auxilio Nacional',
                    'CNJ':'Cosignacion Judicial','RCS':'Recurso De Casacion','INC':'Incidente Concursal','MC':'Medidas Cautelares',
                    'CN':'Conciliacion'}
        numspan={"1":"uno","2":"dos","3":"tres","4":"cuatro","5":"cinco","6":"seis","7":"siete","8":"ocho","9":"nueve","10":"diez",
        ...
        for fi,fr in fgdf.iterrows():
            ...
            if(fr['group_predicted_class']=='N1')or(fr['group_predicted_class']=='N3')or(fr['group_predicted_class']=='N4'):
                for ki,kr in extKeywords[(extKeywords['decision_type'].str.contains('AMOUNT'))
                                         &(extKeywords['fileclass']==fr['group_predicted_class'])].iterrows():
        ...
        #         for ki,kr in extKeywords[(extKeywords['notification_type'].str.contains('TIME FRAME'))
        #&(extKeywords['fileclass']==fr['group_predicted_class'])].iterrows():
            ...
            if (fr['group_predicted_class']=='N5')|(fr['group_predicted_class']=='N6')|(fr['group_predicted_class']=='N9'):
                ...
                for ki,kr in extKeywords[~(extKeywords['decision_type'].str.contains('TIME FRAME'))
                                         &(extKeywords['fileclass']==fr['group_predicted_class'])].iterrows():
                    for i,r in fdf[(fdf['filename'].isin(fr['files']))&(fdf['filetype']=='NOTIFICATION')].iterrows():
                        text=''.join(r['text_response'].split())
                        if r['text_response'][:5]!='Error':
                            ...
                            k1=' '.join(kr['keyword'][0].split())    
                            if f :
                                fgdf.loc[fi,fr['group_predicted_class']+'-Extraction\
                                ']=unidecode.unidecode(str([dict({kr['notification_type']:unidecode.unidecode(str(kr['keyword']))})]))
        ...
        #                 print(r['filename'],js["table_1"][[x for x in js["table_1"].keys() if 'Fecha' in x][0]],
        #table2['1'][[x for x in table2['1'].keys() if 'Fecha' in x][0]])
                        c+=1
    ...
    #             fidlis.append(insertpg_file(f,fg))
                cur1 = conn.cursor()
                cur1.execute("insert into file_info (file_group_id,file_data_id,\
                             file_name) values ("+fg_id+","+file_data_id+","+f+") RETURNING id;")
                f_info_id = cur1.fetchone()[0]
                print(f_info_id, "file_group_id")
